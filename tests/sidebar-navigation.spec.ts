...
test.describe("Enhanced Sidebar Navigation UI Tests", () => {
  ...
  test("4. Enhanced Features - Hover Effects and Animations", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    await page.waitForTimeout(3000);
    ...
    const hoverableElements = await page.$$(
      'nav a, .sidebar a, [class*="nav"] a, button'
    );

    console.log(
      `Testing hover effects on ${hoverableElements.length} elements`
    );
    ...
    const collapsibleElements = await page.$$(
      '[aria-expanded], .collapsible, [class*="expandable"]'
    );
    ...
    for (const testPage of testPages) {
      ...
      for (const selector of breadcrumbSelectors) {
        ...
        if (element) {
          ...
          console.log(
            `Found breadcrumb on ${testPage} with selector: ${selector}`
          );
          break;
        }
      }
  ...
  test("7. Visual Elements and Styling", async ({ page }) => {
    ...
    for (const testPage of pages) {
      try {
        ...
        console.log(
          `Error taking visual quality screenshot for ${testPage}: ${error}`
        );
      }
    }
